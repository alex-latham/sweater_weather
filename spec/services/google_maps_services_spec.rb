...
RSpec.describe GoogleMapsServices do
  ...
  it 'can retrieve location json for paris' do
    VCR.use_cassette('paris geocoding json') do
  ...
  it 'can retrieve location json for paris' do
    VCR.use_cassette('tokyo geocoding json') do
      location_json = GoogleMapsServices.new.get_location('tokyo,jp')

      expect(location_json[:status]).to eq('OK')
      expect(location_json[:results][0][:address_components][0][:short_name]).to eq('Tokyo')
      expect(location_json[:results][0][:address_components][0][:types]).to eq(['administrative_area_level_1', 'political'])
      expect(location_json[:results][0][:address_components][1][:long_name]).to eq('Japan')
      expect(location_json[:results][0][:address_components][1][:types]).to eq(['country', 'political'])
      expect(location_json[:results][0][:formatted_address]).to eq('Tokyo, Japan')
      expect(location_json[:results][0][:geometry][:location][:lat]).to eq(35.6761919)
      expect(location_json[:results][0][:geometry][:location][:lng]).to eq(139.6503106)
    end
  end

  it 'can retrieve location json for washington d.c.' do
    VCR.use_cassette('washington d.c. geocoding json') do
